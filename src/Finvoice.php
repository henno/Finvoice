...
class Finvoice
{
    ...
    public function getFinvoice()
    {
        ...
        $invoiceDetails->addChild('BuyerReferenceIdentifier', isset($this->settings->invoice->buyer_reference) ? $this->settings->invoice->buyer_reference : null);
        ...
        foreach ($this->settings->invoice->rows as $row) {
            ...
            $vatAmount = $row->price * ($row->vat / 100) * $row->amount;
        ...
        $invoiceDetails->addChild('InvoiceTotalVatExcludedAmount', number_format($totalVatExcludedAmount, 2, ',', ''))->addAttribute('AmountCurrencyIdentifier', 'EUR');
        $invoiceDetails->addChild('InvoiceTotalVatAmount', number_format($totalVatAmount, 2, ',', ''))->addAttribute('AmountCurrencyIdentifier', 'EUR');
        $invoiceDetails->addChild('InvoiceTotalVatIncludedAmount', number_format($totalVatIncludedAmount, 2, ',', ''))->addAttribute('AmountCurrencyIdentifier', 'EUR');
        ...
        foreach ($this->settings->invoice->rows as $row) {
            ...
            $invoiceRow->addChild('UnitPriceVatIncludedAmount', number_format($row->price * (100 + $row->vat) / 100, 2, ',', ''))->addAttribute('AmountCurrencyIdentifier', 'EUR');
            if ($row->id > 0) {
        ...
        $epiPaymentInstructionDetails->addChild('EpiInstructedAmount', number_format($totalVatIncludedAmount, 2, ',', ''))->addAttribute('AmountCurrencyIdentifier', 'EUR');
